...
async function showVerseBlocks(ref, text, version, path, holdMs = getSlideHoldMs(), sessionId = currentZapSessionId) {
...
function playVoiceFromCdn(range, verses = []) {
  if (currentAudio) {
  ...
  const verDir    = String(range.bible_version || 'LSG').toLowerCase();
  ...
  return new Promise((resolve) => {
    ...
    function playNext() {
      ...
      if (verDir === 'LSG') audio.playbackRate = 0.85;
...
async function runPsalmsLoop(mySessionId) {
  ...
  while (mySessionId === currentZapSessionId) {
    ...
    const blockDisplay = showVerseBlocks(ref, text || '(Loading…)', 'LSG', jsonPath, holdMs, mySessionId);
    document.getElementById('np-label').textContent = ref;
    addLog(ref, 'dim');
    const r = { book: 'Psalms', bible_version: 'LSG', chapter_start: psalmNum, verse_start: 1 };
    await Promise.all([playVoiceFromCdn(r, verses), blockDisplay]);
...
async function zapChannel(channelId) {
  ...
  while (mySessionId === currentZapSessionId) {
    ...
    const activeVersion  = forcedOverride || r.bible_version || 'LSG';
    const jsonPath       = '/data/bibles/lsg.json';
    ...
    const text = verses.map(v => v.text).join(' ');
    const blockDisplay = showVerseBlocks(ref, text, activeVersion, jsonPath, holdMs, mySessionId);
    hideBibleKeyboard();
...
// BIBLE_CAPS French aliases: LSG JSON uses singular Psaume, while UI may display Psaumes.
if (typeof EN_TO_FR_BOOK !== 'undefined') {
  Object.entries(EN_TO_FR_BOOK).forEach(([en, fr]) => {
    if (BIBLE_CAPS[en] && fr) BIBLE_CAPS[fr] = BIBLE_CAPS[en];
  });
}
BIBLE_CAPS.Psaume = BIBLE_CAPS.Psalms;
BIBLE_CAPS.Psaumes = BIBLE_CAPS.Psalms;
...
async function renderKeyboardMatrix() {
  ...
  const activeVersion = window.SC_FORCE_LANG || 'LSG';
  ...
  /* ── STEP 3 : VERSES ──────────────────────────────── */
    ...
    try {
      const jPath    = '/data/bibles/lsg.json';
...
function getCurrentManualRange() {
  return {
    ...
    bible_version: (window.SC_FORCE_LANG === 'LSG') ? 'LSG' : 'LSG',
...
function lookupPlayNow() {
  ...
  const ver = (window.SC_FORCE_LANG === 'LSG') ? 'LSG' : 'LSG';
  _runLookupRange({
...
async function _runLookupRange(r) {
  const ver      = String(r.bible_version || 'LSG').toUpperCase();
  const jsonPath = '/data/bibles/lsg.json';
  let bibleData;
  try { bibleData = await fetchJSON(jsonPath); }
  ...
  const text = verses.map(v => v.text).join(' ');
  const blockDisplay = showVerseBlocks(ref, text, ver, jsonPath);
  hideBibleKeyboard();
  await Promise.all([playVoiceFromCdn(r, verses), blockDisplay]);
}
